...
type LobbyData = {
  trip: { slug: string; name: string; status: string; maxPlayers: number }
  count: number
  players: Array<{ id: string; name: string; handicap: number }>
  teams: Array<{ id: string; name: string; players: Array<{ id: string; name: string; handicap: number; isCaptain: boolean }> }>
...
export function LobbyView({ slug }: { slug: string }) {
  ...
  async function load(quiet = false) {
    const res = await fetch(`/t/${slug}/api/lobby`, { cache: 'no-store' })
    if (res.ok) {
      setData(await res.json())
      setError('')
    } else if (!quiet) {
      setError('Lobby unavailable right now.')
    }
  }
      ...
      <div className="mx-auto max-w-md space-y-4">
        <section className="rounded-[30px] bg-slate-950 p-5 text-white shadow-sm">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-300">
            {teamsReady
              ? 'Rosters are set. Jump to the live views or review who is in.'
              : 'Players join here before the admin creates teams and pairings.'}
          </p>
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 border-t border-slate-200 bg-white/95 backdrop-blur">
      <div className="mx-auto grid max-w-md grid-cols-4 gap-2 px-4 py-3 text-sm font-bold">
        {items.map((item) => (
          <Link key={item.key} href={item.href} className={`rounded-2xl px-3 py-3 text-center ${active === item.key ? 'bg-slate-950 text-white' : 'text-slate-600'}`}>
            {item.label}
          </Link>
        ))}
      </div>
